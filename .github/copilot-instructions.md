...
**JOSS review concluded**: The paper was accepted (2026-08-30, DOI [10.21105/joss.10403](https://doi.org/10.21105/joss.10403)). `dev/ongoing-work` was merged into `main` on 2026-08-31 and all work now happens directly on `main`.

**For AI assistants**:
- Commit new fixes/features directly to `main`
- Editable install automatically tracks the current branch — just restart kernel after changes
- No need to create `dev/ongoing-work` or other holding branches going forward
- `dev/ongoing-work` remains in history/remote for reference but is no longer the active branch
...
| Sep 2026 | **LKM mass-transfer floor** (`molass-legacy` `Optimizer/PenaltyUtils.py::compute_lkm_mass_transfer_penalty`, `ObjectiveFunctions/G1400.py`): SAMPLE5 (BSA, 4-comp) BH/DE runs showed components 0/1 fitting unrealistically broad, with *which* component collapses swapping between BH and DE — a sign of an unconstrained direction, not real chemistry. Root cause traced to `k_MT` (mass-transfer rate) collapsing toward zero for low-abundance components (no lower bound existed, unlike `R`'s enforced `R>=1.05`). Derived `sigma^2 = 2*t0^2*R^2/Pe + 2*t0*(R-1)/k_MT` analytically from `LkmLinear.py`'s transfer function (Taylor-expanding `ln H(s)` to `O(s^2)`) — verified against real fitted data to <1%. Gives `k_MT_floor = (R-1)*Pe/((m^2-1)*t0*R^2)`; added as a dimensionless soft-ramp penalty (`m=1.5` default, sensitivity-checked against the real data — the two pathological components fail under any `m` up to 3.0, the well-behaved monomer never comes close to failing). `G1400.get_score_names()` overridden (mirrors `G0367.py`'s pattern) to insert `"mass_transfer_penalty"` in the named breakdown — without it, `get_score_breakdown()`/`diagnose()` would silently misalign. 11/11 unit tests pass (`molass-legacy/tests/test_lkm_mass_transfer_penalty.py`); **not yet validated against a real BH/DE run** (unit-level only so far). Full derivation, sensitivity table, and open follow-ups: `Copilot/refactor/DESIGN_lkm_mass_transfer_floor.md`. Source: `molass-papers/experiments/recipe_runner_notebook_redesign.ipynb` Sections 7-12. |
