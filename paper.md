...
The MOLASS library has demonstrated scientific value through research conducted using its predecessor GUI tool [@Jiang:2023; @Furukawa:2025], as well as through its contribution to the development of SEC-SAXS data analysis systems at synchrotron facilities [@Yonezawa:2023]. In SEC-SAXS experiments, component decomposition techniques are required when significant interparticle interference occurs or chromatographic peaks overlap. MOLASS is one of several software packages developed to address this challenge and is regarded as providing a rigorous peak decomposition framework [@Matsui:2024]. CHROMIXS defers the analysis of overlapping chromatographic peaks to “other methods” [@Panjkovich:2018], whereas EFAMIX defines quantitative conditions for reliable separation (signal-to-noise ratio ≥10³ for three-component separation; peak asymmetry parameter τ≤2; baseline width separation ≥2× for reliable decomposition) [@Konarev:2021]. REGALS employs a two-stage framework combining EFA with L1 regularization [@Meisburger:mf5050] and is therefore guided by assumptions underlying EFA in strongly overlapping cases. The MOLASS library approaches overlapping peak decomposition through explicit parametric modeling of elution curves combined with global optimization. The solution space is constrained using physicochemical consistency derived from SEC theory (size-dependent elution behavior) and SAXS analysis (consistency of radius of gyration across elution fractions). This approach provides an alternative and complementary algorithmic strategy for modeling overlapping chromatographic peaks.
