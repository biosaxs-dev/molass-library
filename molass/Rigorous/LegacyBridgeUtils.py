...
def construct_legacy_optimizer(dsets, baseline_objects, spectral_vectors, num_components=3, model="EGH", method="BH", for_split_only=False, function_code=None, debug=False):
    ...
    if model == "EGH":
        ...
        if model.startswith("SDM"):
            ...
            pass  # GrmParams derives bounds from init params directly; no legacy estimator needed
    
    return optimizer

def prepare_rigorous_folders(decomposition, rgcurve, analysis_folder=None, data_ssd=None, debug=False, export_npy=True):
    ...
    # subprocess-only overrides: skip entirely for in_process=True runs
    if export_npy:
    ...
    _ds_exported = False
    if export_npy and hasattr(uv_base_curve, 'diff_spline') and uv_base_curve.diff_spline is not None:
