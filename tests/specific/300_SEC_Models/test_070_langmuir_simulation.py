...
def test_wall_bound_zero_when_no_adsorption():
    """k_a=0 → no wall-adsorbed steps, K_ads=0."""
    s = _sim(k_a=0.0, k_d=1.0)
    assert s['wall_bound'].sum() == 0
    assert s['K_ads'] == 0.0


def test_wall_bound_nonzero_with_adsorption():
    """k_a>0 → some wall-adsorbed steps (uses seed=42 that reliably enters pores)."""
    s = _sim(k_a=3.0, k_d=1.0, n_steps=10_000, seed=42)
    assert s['wall_bound'].sum() > 0


def test_wall_bound_only_inside_pore():
    """wall_bound can only be True when the molecule is in a pore (states >= 0)."""
    s = _sim(k_a=2.0, k_d=1.0)
...
def test_K_ads_theory():
    """K_ads = k_a / k_d."""
    k_a, k_d = 3.0, 2.0
    s = _sim(k_a=k_a, k_d=k_d)
    assert abs(s['K_ads'] - k_a / k_d) < 1e-10
...
def test_mean_dwell_longer_with_adsorption():
    ...
    s_sec = _sim(n_steps=10_000, k_a=0.0, k_d=1.0, seed=42)
    s_ads = _sim(n_steps=10_000, k_a=3.0, k_d=1.0, seed=42)
...
def test_return_shapes():
    ...
    n = 3_000
    s = _sim(n_steps=n, k_a=1.0, k_d=1.0)
    assert s['wall_bound'].shape == (n + 1,)
    assert s['positions'].shape == (n + 1, 2)
    assert s['states'].shape == (n + 1,)


def test_k_d_zero_yields_inf_K_ads():
    """k_d=0 → K_ads = inf (molecule never desorbs)."""
    s = _sim(k_a=1.0, k_d=0.0)
    assert s['K_ads'] == float('inf')
